...
describe('RoundRobinService', () => {
  ...
  describe('Algorithm Verification', () => {
    it('returns null if paused', async () => {
      prisma.roundRobinState.findUnique.mockResolvedValue({ id: 'singleton', isPaused: true });
    ...
    it('returns null if no eligible active counsellors', async () => {
      prisma.roundRobinState.findUnique.mockResolvedValue({ id: 'singleton', isPaused: false });
    ...
    it('returns the next counsellor in sequence', async () => {
      prisma.roundRobinState.findUnique.mockResolvedValue({ id: 'singleton', isPaused: false, lastAssignedUserId: 'user-a' });
      prisma.roundRobinCounsellorState.findMany.mockResolvedValue([
      ...
      const nextUser = await service.getNextAssignee(prisma);
      expect(nextUser).toBe('user-b');
      expect(prisma.roundRobinState.update).toHaveBeenCalledWith({
        where: { id: 'singleton' },
        data: { lastAssignedUserId: 'user-b' },
      });
    });

    it('gives priority to returning counsellor', async () => {
      prisma.roundRobinState.findUnique.mockResolvedValue({ id: 'singleton', isPaused: false, lastAssignedUserId: 'user-c' });
